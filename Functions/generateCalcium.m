function [dF,Events,Cliques]=generateCalcium(dM,tM,nCliques)
    Noise=0.02; %deltaF noise level
    corupt=0.05; %Corruption index
    ...
    function [dF,cPk]=CalciumEvent(dF,tme,n,cPk)
        ...
        for tN=1:2*eL
            if tN<=eL
                eForm(tN)=((3+rand())/4)*Pk*tN/eL;
            else
                eForm(tN)=((3+rand())/4)*Pk*(tN-2*eL)/(2*eL);
            end
        end
        [~,c]=max(eForm);
